...
function normalizeCandidateWithImage(
  candidate: RecipeCandidate,
  html: string,
  sourceUrl: URL,
): ExtractedRecipe | null {
  const normalized = normalizeCandidate(candidate, sourceUrl);
  if (!normalized || normalized.imageUrl) return normalized;

  const fallbackImageUrl = extractFallbackImage(
    html,
    sourceUrl,
    normalized.recipeName,
  );
  if (!fallbackImageUrl) return normalized;
  return {
    ...normalized,
    imageUrl: resolveImageUrl(fallbackImageUrl, sourceUrl),
  };
}
...
function attemptRecipeExtraction(
  ...
  const normalizedJsonLd = jsonLdCandidate
    ? normalizeCandidateWithImage(jsonLdCandidate, html, sourceUrl)
    : null;
  ...
  const tryFallback = (
    ...
    const normalized = normalizeCandidateWithImage(candidate, html, sourceUrl);
