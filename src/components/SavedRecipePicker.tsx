...
import { ArrowLeft, ImageOff, ThumbsDown, ThumbsUp, X } from "lucide-react";
import type { SavedRecipe } from "../types";
import { fetchSavedRecipes } from "../lib/supabase";

export default function SavedRecipePicker({ isLoggedIn, onBack, onClose, onSelect }: {
  isLoggedIn: boolean;
  onBack: () => void;
  onClose: () => void;
  ...
  useEffect(() => {
    ...
    void fetchSavedRecipes().then((saved) => {
      if (saved) {
        setRecipes([...saved].sort((a, b) =>
          Number(a.userRating === "disliked") - Number(b.userRating === "disliked")
        ));
      }
      setLoading(false);
    });
  ...
  return (
    <div className="w-full overflow-hidden rounded-[24px] bg-[#FAF9F5]">
      <header className="flex items-center justify-between border-b border-[#EDEADF] bg-white px-4 py-3">
        <button
          type="button"
          onClick={onBack}
          aria-label="Tillbaka till manuell måltid"
          className="flex h-10 w-10 items-center justify-center rounded-full bg-[#FAF9F5] text-gray-500 transition-transform active:scale-[0.96]"
        >
          <ArrowLeft className="h-5 w-5" />
        </button>
        <div className="min-w-0 flex-1 px-3">
          <p className="text-[10px] font-bold uppercase tracking-wider text-emerald-700">Veckoplanering</p>
          <h2 id="saved-recipe-picker-title" className="mt-0.5 truncate text-lg font-bold text-gray-900">Välj ett sparat recept</h2>
        </div>
        <button
          type="button"
          onClick={onClose}
      ...
      <div className="max-h-[62vh] overflow-y-auto p-4" style={{ WebkitOverflowScrolling: "touch" }}>
        {!isLoggedIn ? (
          ...
          <div className="grid grid-cols-2 gap-3">
            {recipes.map((recipe) => (
              <button
                ...
                className="overflow-hidden rounded-2xl bg-white text-left shadow-sm ring-1 ring-[#EDEADF] transition-transform active:scale-[0.98]"
              >
                <div className="relative flex aspect-[4/3] items-center justify-center overflow-hidden bg-gradient-to-br from-emerald-50 to-amber-50 text-emerald-700">
                  ...
                  {recipe.userRating && (
                    <span className={`absolute right-2 top-2 flex h-7 w-7 items-center justify-center rounded-full bg-white/90 shadow-sm ${recipe.userRating === "liked" ? "text-emerald-700" : "text-amber-700"}`}>
                      {recipe.userRating === "liked" ? <ThumbsUp className="h-3.5 w-3.5" /> : <ThumbsDown className="h-3.5 w-3.5" />}
                    </span>
                  )}
                </div>
